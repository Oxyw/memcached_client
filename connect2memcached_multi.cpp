...
std::string file_stats_slabs = "stats_slabs";
std::string file_result = "result";
...
void save_stats_slabs(size_t cnt, int fidx)
{
    int port = fidx + PORT;
    ...
    // save log
    std::string file_name = file_stats_slabs + std::to_string(port);
    std::ofstream outf(file_name, std::ofstream::app);
    if (!outf)
    {
        std::cout << "Fail to open the output file!" << std::endl;
        exit(1);
    }
    outf << "total:" << cnt << ", req:" << reqs[fidx] << std::endl;
    int sl = BUFS - 1;
    bool flag = false;
    do // loop receiving
    {
...
int main(int argc, char *argv[])
{
    ...
    memcached_st *memc[N];
    memcached_return rc;
    memcached_server_st *server[N];

    for(int i = 1; i < argc; i++)
    {
        memc[i] = memcached_create(NULL);
        server[i] = memcached_server_list_append(NULL, IP, (PORT + i), &rc);
        rc = memcached_server_push(memc[i], server[i]);
        memcached_server_list_free(server[i]);
    }
    ...
    // output
    std::ofstream outFile(file_result, std::ofstream::out);
    if (!outFile)
    {
        std::cout << "Fail to open the output file!" << std::endl;
        exit(1);
    ...
    std::priority_queue<std::pair<uint32_t, uint32_t>, std::vector<std::pair<uint32_t, uint32_t>>, cmp> pq; // 小根堆
    ...
    while (!pq.empty())
    {
        ...
        {
            // Get data
            char *result = memcached_get(memc[fidx], key.c_str(), key.length(), &value_length, &flag, &rc);
            /*
            ...
            */
            ...
            else // need set to simulate the access to the database
            {
                ...
                rc = memcached_set(memc[fidx], key.c_str(), key.length(), v.c_str(), v.length(), expiration, (fidx | SET_DB));
            }   
            free(result);
        }
        else if (op == "set")
        {
            ...
            rc = memcached_set(memc[fidx], key.c_str(), key.length(), value.c_str(), value.length(), expiration, fidx);
        }
        else if (op == "incr")
        {
            uint32_t offset = stoi(std::string(value_length, '1'));
            uint64_t re_val;
            memcached_increment(memc[fidx], key.c_str(), key_length, offset, &re_val);
        }
        else if (op == "add")
        {
            std::string value(value_length, '1');
        ...
        {
            rc = memcached_delete(memc[fidx], key.c_str(), key_length, expiration);
        }
        ...
        if (total_count % 1000000 == 0)
        {
            ...
            {
                for (int i = 1; i < argc; i++)
                    save_stats_slabs(total_count, i);
            }
        }
    }
